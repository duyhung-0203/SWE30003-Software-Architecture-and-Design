    ...
    Promise.all([getCoordinates(startAddress), getCoordinates(endAddress)])
        .then(results => {
            ...
            routingControl = L.Routing.control({
                ...
                document.getElementById('completeButton').style.display = "initial";
...
document.getElementById('completeButton').addEventListener('click', function (e) {
    ...
    $.ajax({
        type: 'POST',
        url: 'controllers/complete_ride.php',
        data: { status: status },
        success: function (response) {
            if (response.success) {
                document.getElementById('completeButton').outerHTML = `<h2>Ask your passenger for payment ...</h2>`;
            } else {
                console.log("fail")
            }
        },
